...
CCSystemInfo::CCSystemInfo() {
}

QStringList CCSystemInfo::get_mac() {
...
QString CCSystemInfo::get_mac_frist() {
    QList<QNetworkInterface> ifaces = QNetworkInterface::allInterfaces();
    for(int i = 0; i < ifaces.count(); i++) {
        QNetworkInterface iface = ifaces.at(i);
        //过滤掉本地回环地址、没有开启的地址
...
void CCSystemInfo::getcpuid(unsigned int CPUInfo[], unsigned int InfoType) {
...
void CCSystemInfo::getcpuidex(unsigned int CPUInfo[], unsigned int InfoType, unsigned int ECXValue) {
...
#endif
QString CCSystemInfo::get_cpuId() {
    QString cpu_id = "";
